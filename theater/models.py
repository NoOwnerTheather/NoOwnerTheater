...
class ReleasedMovie(models.Model):
    ...
    rating = models.FloatField(verbose_name='평균평점', default=0)
...
class Review(models.Model):
    user = models.ForeignKey(User, on_delete=models.CASCADE)
    movie = models.ForeignKey(ReleasedMovie, on_delete=models.CASCADE)
...
class CommentPreview(models.Model):
    user = models.ForeignKey(User, on_delete=models.CASCADE) 
    movie = models.ForeignKey(UnreleasedMovie, on_delete=models.CASCADE) 
    content = models.TextField(verbose_name='내용')
    like = models.IntegerField(verbose_name='좋아요', default=0)
    created_at = models.DateTimeField(verbose_name='작성일', auto_now_add=True)
